...
void Octree::create(const ofMesh & geo, int numLevels) {
	...
	colors.push_back(ofColor::lightPink);
...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	//stop if we are at the final level or if the node becomes too small to hold and points
	...
	//go through each of the subboxes
	for (int i = 0; i < 8; i++) {
		TreeNode child;
		child.box = boxes[i];

		vector<int> childPoints;
		int count = getMeshPointsInBox(mesh, node.points, child.box, childPoints);
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
	...
	//if we are on a leaf node then we stop recursing
	if (node.points.size() == 1) {
		//store this box to be printed
		boxListRtn.push_back(node.box);
		return true;
	}

	//recurse on child nodes to get the smallest box intersected
	for (auto & child : node.children) {
